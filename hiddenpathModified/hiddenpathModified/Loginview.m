...
{	
	...
	if([self validateEmail:txt_mo.text])
	{
		[AlertHandler showAlertForProcess];
		NSMutableURLRequest *request=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://openxcellaus.info/hiddenpath/user_action.php?action=forgot_password&vEmail=%@",txt_mo.text]]];
...
-(IBAction)btn_login_clicked:(id)sender
{
    ...
    if([self validateEmail:[user_name text]] ==1)
    {
     ...
     //   [alert2 release];
        ...
        NSString *registrationWS = [NSString stringWithFormat:@"http://openxcellaus.info/hiddenpath/user_action.php?action=login&vEmail=%@&vPassword=%@",user_name.text,txt_password.text];
...
-(void)parsingResponse:(NSDictionary*)dictio
{
    ...
    if ([[dictio valueForKey:@"msg"]isEqualToString:@"Login Successfull"]) {
        
     iscurrect=[obj_databasemethod UserNameAndPasswordSerching_FromLogin:user_name.text passwordString:txt_password.text];
        iscurrect =TRUE;
