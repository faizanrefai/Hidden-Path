...
-(void)getGoalParsing1:(NSDictionary*)dic{
    
    Ary_goal=[dic objectForKey:@"goals"];
}
-(void)getGoalParsing2:(NSDictionary*)dic{
    
    [AlertHandler hideAlert];
    Ary_Affirmation=[dic objectForKey:@"affirmations"];

    [self.tbl_journal reloadData];
	...
	if(isMaster){		
		...
		for (str1 in Selecteddate_Ary ){
			...
			if([str isEqualToString:str1]){
                ...
                NSString *urlStr2 = [NSString stringWithFormat:@"http://openxcellaus.info/hiddenpath/affirmation_action.php?action=show_affirmation&iUserId=%d",[[[NSUserDefaults standardUserDefaults]valueForKey:@"userID"]intValue]];
                
                NSMutableURLRequest *request2=[NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlStr2]];
                
                JSONParser*  parser2 = [[JSONParser alloc] initWithRequestForThread:request2 sel:@selector(getGoalParsing2:) andHandler:self];
